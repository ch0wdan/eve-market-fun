...
function priceAsFloat (order) {
    return parseFloat(order.price, 10);
}
    ...
    updateFromMarketData: function (type_id, region_id) {
        ...
        return exports.MarketDataRaws.forge().query(function (qb) {
            ...
            _.each(by_station, function (orders, station_id) {
                ...
                var sell_orders = _.sortBy(by_bid[false], function (order) {
                    return parseFloat(order.price, 10);
                });
                var buy_orders = _.sortBy(by_bid[true], function (order) {
                    return parseFloat(order.price, 10);
                }).reverse();
                    ...
                    return Promise.all(_.map(LEAD_COMPARISONS, function (cmp) {
                        ...
                        if (from_price < to_price) {
                            ...
                            return Promise.all([
                                ...
                                if (!cmp.to_bid) {
                                    curr_to.volRemaining = 999999999999;
                                }
                                if (cmp.from_bid) {
                                    curr_from.volRemaining = 999999999999;
                                }
                                ...
                                while (curr_from && curr_to) {
                                    ...
                                    var vol = (curr_from.volRemaining > curr_to.volRemaining) ?
                                ...
                                var total_volume = _.reduce(trades, function (memo, trade) {
                                    return memo + trade.volume;
                                }, 0);
                                var total_investment = _.reduce(trades, function (memo, trade) {
                                    return memo + (trade.from_price * trade.volume); 
                                }, 0);
                                var total_sales = _.reduce(trades, function (memo, trade) {
                                    return memo + (trade.to_price * trade.volume); 
                                }, 0);
                                var total_profit = total_sales - total_investment;
                                var isk_per_m3 = total_profit / (item_data[0].volume * total_volume);
