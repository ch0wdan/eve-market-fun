...
exports.processEmuu = function (market_data) {
    _.chain(market_data.rowsets).map(function (rowset) {
        ...
        logger.info("%s market %s received for %s from %s, %s queued",
            rowset.rows.length, market_data.resultType,
            rowset.typeID, rowset.regionID, queue.length());
    });
};
...
var emuuQueue = async.queue(function (task, next) {
    ...
    var orders = _.chain(task.rows)
        .filter(function (row) {
            if (row.orderState != 0) { return false; }
            return true;
        })
        .value();
    if (!orders.length) { return next(); }

    db('MarketOrders').where({
        typeID: task.typeID,
        regionID: task.regionID 
    }).del().then(function () {
        return db('MarketOrders').insert(orders);
    }).then(function () {
...
// For now, we'll only look for leads to major trade hubs.
var TRADE_HUB_STATION_IDS = exports.TRADE_HUB_STATION_IDS = [
...
var filterOrdersByStation = function (orders, includeNonHubs) {
    return _.chain(orders).filter(function (row) {
        // Consider only open orders
        if (row.orderState != 0) { return false; }
        // For now, only analyze orders between trade hubs.
        if (!includeNonHubs && TRADE_HUB_STATION_IDS.indexOf(row.stationID) === -1) {
            return false;
        }
        return true;
    }).map(function (row) {
        row.bid = !!row.bid;
        return row;
    }).groupBy('stationID').value();
};

var marketMarginQueue = async.queue(function (task, next) {
    var by_station = filterOrdersByStation(task.rows, true);
    ...
    Promise.all(models).catch(function (e) {
        logger.error("Market order import error", e);
        logger.error(util.inspect(e));
    }).then(function () {
        marketTradeLeadsQueue.push(task);
    }).finally(next);

}, 1);

// Set up a table of comparisons to make between stations.
var LEAD_COMPARISONS = [];
[true, false].forEach(function (from_bid) {
    [true, false].forEach(function (to_bid) {
        ['updated', 'compare'].forEach(function (from_name) {
            var to_name = ('updated' == from_name) ? 'compare' : 'updated';
...
// This queue populates a second queue with station-to-station trade
// comparisons based on newly arrived market order updates.
var marketTradeLeadsQueue = exports.marketTradeLeadsQueue = async.queue(function (task, next) {
    var db = exports.db_Main;

    var by_station = filterOrdersByStation(task.rows, task.scanNonHubs);

    db('MarketOrders').select()
    .where({
        typeID: task.typeID,
        orderState: 0
    })
    .whereIn('stationID', TRADE_HUB_STATION_IDS)
    .then(function (all_hub_orders) {

        var orders_by_hub = filterOrdersByStation(all_hub_orders);

        _.each(orders_by_hub, function (hub_orders, hub_station_id) {
        
            var hub_orders_by_bid = _.chain(hub_orders)
                .sortBy('price').groupBy('bid').value();
            if (true in hub_orders_by_bid) {
                hub_orders_by_bid[true].reverse();
            }

            var compare = {
                stationID: hub_station_id,
                orders: hub_orders,
                sellOrders: hub_orders_by_bid[false],
                buyOrders: hub_orders_by_bid[true]
            };

            _.each(by_station, function (rows, station_id) {

                if (station_id == hub_station_id) { return; }

                var updated_orders_by_bid = _.chain(rows)
                    .sortBy('price').groupBy('bid').value();
                if (true in updated_orders_by_bid) {
                    updated_orders_by_bid[true].reverse();
                }

                marketInterStationLeadsQueue.push({
                    typeID: task.typeID,
                    compare: compare,
                    updated: {
                        stationID: station_id,
                        orders: rows,
                        sellOrders: updated_orders_by_bid[false],
                        buyOrders: updated_orders_by_bid[true]
                    }
                });

            });

            if (marketInterStationLeadsQueue.length() > 0 && !task.squelchQueueStatus) {
                logger.verbose("Inter-hub trade leads, queued " +
                    marketInterStationLeadsQueue.length());
            }

        });
    }).catch(function (e) {
        logger.error("TRADE LEAD ERROR " + e);
    })
...
// This queue performs station-to-station comparisons to discover trade leads.
var marketInterStationLeadsQueue = async.queue(function (task, next) {
    var db = exports.db_Main;
    var out = [];

    LEAD_COMPARISONS.forEach(function (cmp) {
    
        var from = task[cmp.from_name];
        var from_stationid = from.stationID;
        var from_orders = from[cmp.from_bid ? 'buyOrders' : 'sellOrders'];
        if (!from_orders || !from_orders.length) { return; }

        var to = task[cmp.to_name];
        var to_stationid = to.stationID;
        var to_orders = to[cmp.to_bid ? 'buyOrders' : 'sellOrders'];
        if (!to_orders || !to_orders.length) { return; }

        var from_top_order = from_orders[0];
        var to_top_order = to_orders[0];

        var from_price = from_top_order.price;
        var to_price = to_top_order.price;
        ...
        out.push({
            ...
            fromRegionID: from_top_order.regionID,
            fromSolarSystemID: from_top_order.solarSystemID,
            fromStationID: from_stationid,
            fromBid: cmp.from_bid,
            fromTopOrderID: from_top_order.orderID,

            toRegionID: to_top_order.regionID,
            toSolarSystemID: to_top_order.solarSystemID,
            toStationID: to_stationid,
            toBid: cmp.to_bid,
            toTopOrderID: to_top_order.orderID
        });
    ...
    // Delete all leads involving the compared stations for this type, before
    // performing the batch insert of discovered trades.
    db('MarketTradeLeads')
        .where('typeID', '=', task.typeID)
        .andWhere(function () {
            this.orWhere(function () {
                this.where('fromStationID', '=', task.updated.stationID)
                    .where('toStationID', '=', task.compare.stationID)
            });
            this.orWhere(function () {
                this.where('fromStationID', '=', task.compare.stationID)
                    .where('toStationID', '=', task.updated.stationID)
            });
        }).del().then(function () {
            return db('MarketTradeLeads').insert(out);
        }).catch(function (e) {
            logger.error("TRADE LEAD ERROR " + e);
        }).finally(next);
