...
var trade_hub_solar_system_ids = [
    30000142, // Jita
    30002053, // Hek
    30002187, // Amarr
    30002510, // Rens
...
var marketLeadsQueue = async.queue(function (task, next) {
    var db = exports.db_Main;
    db.transaction(function (t) {
        db('MarketTradeLeads').transacting(t).where({
            typeID: task.typeID,
            fromRegionID: task.regionID 
        }).del().then(function () {
            return db('MarketTradeLeads').transacting(t).where({
                typeID: task.typeID,
                toRegionID: task.regionID 
            }).del()
        }).then(t.commit, t.rollback);
    }).then(function () {
        ...
        _.each(by_station, function (rows, station_id) {
            ...
            var sell_orders = _.chain(rows).where({bid: false})
                .sortBy('price').value();

            var buy_orders = _.chain(rows).where({bid: true})
                .sortBy('price').reverse().value();

        });

        logger.info("Market trade leads calculated for type " + task.typeID + 
            " in " +
            models.length + " stations - " + 
            marketLeadsQueue.length() + " queued");
