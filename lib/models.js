...
var marketTradeLeadsQueue = async.queue(function (task, next) {
    var db = exports.db_Main;

    var by_station = _.groupBy(task.rows, 'stationID');

    // Look up market orders for this updated item at the known trade hubs
    var hub_lookups = _.chain(trade_hub_station_ids)
        .map(function (station_id) {
            return [
                station_id,
                db('MarketOrders').select().where({
                    typeID: task.typeID,
                    stationID: station_id
                })
            ];
    ...
    // Using the trade hub orders, queue up station-to-station comparisons.
    Promise.props(hub_lookups).then(function (orders_by_hub) {
        _.each(orders_by_hub, function (hub_orders, hub_station_id) {
        
            var hub_sell_orders = _.chain(hub_orders)
                .where({bid: 0}).sortBy('price').value();
            var hub_buy_orders = _.chain(hub_orders)
                .where({bid: 1}).sortBy('price').reverse().value();
            var compare = {
            ...
            _.each(by_station, function (rows, station_id) {
                var updated_sell_orders = _.chain(rows)
                    .where({bid: false}).sortBy('price').value();
                var updated_buy_orders = _.chain(rows)
                    .where({bid: true}).sortBy('price').reverse().value();
            ...
            logger.verbose("Inter-station trade leads, queued " +
...
var marketInterStationLeadsQueue = async.queue(function (task, next) {
    ...
    // First, delete all existing trade leads for this newly updated station.
    db('MarketTradeLeads').where('typeID', '=', task.typeID).andWhere(function () {
        this.where('fromStationID', '=', task.updated.stationID)
            .orWhere('toStationID', '=', task.updated.stationID);
    }).del().then(function () {
        var out = [];
        ...
        LEAD_COMPARISONS.forEach(function (cmp) {
            ...
            out.push(exports.MarketTradeLead.forge({
                typeID: task.typeID,
                ...
                fromStationID: from_top_order.stationID,
                ...
                toStationID: to_top_order.stationID,
                toBid: cmp.to_bid,
...
marketInterStationLeadsQueue.drain = function () {
    logger.verbose("Inter-station trade leads, queue drained.");
}
