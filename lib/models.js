...
var MTL_compareQueue = async.queue(function (task, next) {
    ...
    Promise.props({
        ...
        // TODO: Grab this from a caching source!
        ...
        var from_orders = ordersByBidAndStation(
            from_data.rows(), cmp.from_bid, from_hub.stationID);
        if (!from_orders.length) { return; }

        var to_orders = ordersByBidAndStation(
            to_data.rows(), cmp.to_bid, to_hub.stationID);
        ...
        var isk_per_m3 = totals.profit / (props.item_data[0].volume * totals.volume);
