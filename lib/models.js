...
var MTL_compareQueue = async.queue(function (task, next) {
    var self = task.self;
    var type_id = task.type_id;
    var cmp = task.cmp;
    var a_hub = task.a_hub;
    var b_hub = task.b_hub;

    var b_margin, from_margin, to_margin, from_price, to_price,
        from_station_id, to_station_id, margin, margin_perc;

    Promise.all([
        exports.MarketMargins.forge().query(function (qb) {
            qb.where({ typeID: type_id, stationID: a_hub.stationID });
        }).fetch(),
        exports.MarketMargins.forge().query(function (qb) {
            qb.where({ typeID: type_id, stationID: b_hub.stationID });
        }).fetch()
    ]).spread(function (a_margins, b_margins) {
        var a_margin = a_margins.first();
        if (!a_margin) { return; }

        b_margin = b_margins.first();
        if (!b_margin) { return; }

        from_margin = ('a' === cmp.from_name) ?  a_margin : b_margin;
        to_margin = ('a' === cmp.to_name) ?  a_margin : b_margin;
        
        from_station_id = from_margin.get('stationID');
        to_station_id = to_margin.get('stationID');

        from_price = from_margin.get(cmp.from_bid ? 'maxBuyPrice' : 'minSellPrice');
        to_price = to_margin.get(cmp.to_bid ? 'maxBuyPrice' : 'minSellPrice');

        if (from_price > to_price) { return; }

         margin = to_price - from_price;
         margin_perc = (margin / to_price) * 100;
        ...
        return Promise.props({
            ...
            item_data: exports.db_EVE('invTypes')
        ...
        var from_data = props.from_data;
        var to_data = props.to_data;
        var item_data = props.item_data;
        ...
        if (cmp.from_bid && !cmp.to_bid) {
            // Volume from own buy order to own sell order is notionally infinite.
            trades.push({
