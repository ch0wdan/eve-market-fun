...
var marketMarginQueue = async.queue(function (task, next) {
    var by_station = _.chain(task.rows)
        .filter(function (row) {
            // Consider only open orders
            if (row.orderState != 0) { return; }

            // For now, only analyze orders between trade hubs.
            return task.scanNonHubs ||
                TRADE_HUB_STATION_IDS.indexOf(row.stationID) !== -1
        })
        .groupBy('stationID')
        .value();
...
var marketTradeLeadsQueue = exports.marketTradeLeadsQueue = async.queue(function (task, next) {
    ...
    .then(function (all_hub_orders) {
        ...
        var orders_by_hub = _.groupBy(all_hub_orders, 'stationID');

        _.each(orders_by_hub, function (hub_orders, hub_station_id) {
        
            var hub_sell_orders = _.chain(hub_orders).filter(function (order) {
                return !!!order.bid
            }).where({bid: 0}).sortBy('price').value();
            var hub_buy_orders = _.chain(hub_orders).filter(function (order) {
                return !!order.bid
            }).sortBy('price').reverse().value();
            var compare = {
                stationID: hub_station_id,
                orders: hub_orders,
                sellOrders: hub_sell_orders,
                buyOrders: hub_buy_orders
            };
            ...
            _.each(by_station, function (rows, station_id) {
                ...
                var updated_sell_orders = _.chain(rows).filter(function (order) {
                    return !!!order.bid
                }).sortBy('price').value();
                var updated_buy_orders = _.chain(rows).filter(function (order) {
                    return !!order.bid
                }).sortBy('price').reverse().value();
                ...
                marketInterStationLeadsQueue.push({
                    ...
                    updated: {
                        stationID: station_id,
                        orders: rows,
                        sellOrders: updated_sell_orders,
                        buyOrders: updated_buy_orders
                    }
                });
