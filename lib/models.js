...
function calcVolatility (rows, range) {
    ...
    var vol = (Math.sqrt(avg_deviation) / mean) * 100;
    return vol;
};
...
var emuuHistoryQueue = async.queue(function (task, next) {
    ...
    /*
    db.transaction(function (t) {
        task.rows = _.sortBy(task.rows, 'date');
        task.rows.reverse();
    ...
    */
        ...
        data.volatility = calcVolatility(task.rows);
        data.volatilityForWeek = calcVolatility(task.rows, 7);
        data.volatilityForMonth = calcVolatility(task.rows, 30);
