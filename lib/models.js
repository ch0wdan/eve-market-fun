...
var updateQueue = exports.MarketOrders.updateQueue =
    async.queue(function (row, next) {
        return exports.MarketOrder.forge({
            orderID: row.orderID
        }).createOrUpdate(row).then(function (order) {
            orders_processed++;
            next();
        })
...
updateQueue.drain = function () {
    logger.verbose("%s market orders processed", orders_processed);
    orders_processed = 0;
};

var logEmuu = function (import_type, rowset) {
    ...
    .then(function (items) {
        ...
        logger.info("%s market %s received for %s (%s) from %s (%s)",
            rowset.rows.length, import_type, item.typeName, item.typeID,
            regions[0].regionName, rowset.regionID);
    });
};

exports.MarketOrders.processEmuu = function (market_data) {
    _.chain(market_data.rowsets).map(function (rowset) {
        logEmuu('order', rowset);
        var row_defaults = {
...
var histories_processed = 0;
var historyUpdateQueue = exports.MarketHistories.queue =
    async.queue(function (row, next) {
        return exports.MarketHistory.forge({
            typeID: row.typeID,
            regionID: row.regionID,
            date: row.date
        }).createOrUpdate(row).then(function () {
            histories_processed++;
            next();
        })
        .catch(function (e) { next(e) });
...
historyUpdateQueue.drain = function () {
    logger.verbose("%s market history processed",
        histories_processed);
    histories_processed = 0;
};

exports.MarketHistories.processEmuu = function (market_data) {
    _.chain(market_data.rowsets).map(function (rowset) {
        logEmuu('history', rowset);
        var row_defaults = {
            regionID: rowset.regionID,
            typeID: rowset.typeID
        };
        ...
        exports.MarketHistories.queue.push(row);
    });
};
