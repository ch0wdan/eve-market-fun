...
module.exports = function (prog, init) {
    ...
    function cmdMarketOrdersEMDR () {
        ...
        var t_window = t_start;

        var screen = blessed.screen();

        var root = blessed.box({
        ...
        function renderGui () {
            ...
            _.each(ct, function (amt, name) {
                ...
                ctBoxes[name].setContent([
                    ...
                    Math.round(amt) + ' processed',
                    Math.round(amt / t_minutes) + ' per min'        
                ].join("\n"));
            });
    ...
    var queues = {
        ...
        emdr: async.queue(function (task, next) {
        ...
        history: async.queue(function (task, next) {
        ...
        leads: async.queue(function (task, next) {
...
LogBoxTransport.prototype.log = function (level, msg, meta, callback) {
    ...
    var len = msgs.length;
    if (len > self.history) {
        msgs = msgs.slice(0 - self.history, self.history);
        if (level === 'error') { this.err = msgs; }
        else { this.out = msgs; }
    }
