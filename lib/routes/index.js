            ...
            return orders.mapThen(function (order) {
                ...
                return db_EVE('invTypes').select('typeName')
                    ...
                    .then(function (rows) {
                        ...
                        var type = (order.bid == '1') ? 'buy' : 'sell';
                        out.orders[type].push(order);
                    });
            });
        }).then(function () {
            out.orders.buy = _.sortBy(out.orders.buy, 'typeName');
            out.orders.sell = _.sortBy(out.orders.sell, 'typeName');
        }).catch(function (e) {
            util.debug(e);
        }).finally(function () {
