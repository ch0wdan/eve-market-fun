...
prog.command('characters:update [username]')
    ...
    .action(init(function (username) {
        ...
        .catch(function () {
            ...
            var c_name = character.get('name');
            var char_pk = character.get('id');
            var char_eve_id = character.get('characterID');
            logger.info("Fetching character API data for %s", c_name);
            ...
            logger.info("%s transactions imported for %s",
                saves.transactions.length, saves.character.get('name'));
            logger.info("%s journals imported for %s",
                saves.journals.length, saves.character.get('name'));
            logger.info("%s orders imported for %s",
