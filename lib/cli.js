...
prog.command('marketorders:emdr')
    ...
    .action(init(function () {
        ...
        var MarketDataRaws = models.MarketDataRaws.forge();
        var MarketMargins = models.MarketMargins.forge();
        var MarketTradeLeads = models.MarketTradeLeads.forge();
        var MarketHistoryAggregates = models.MarketHistoryAggregates.forge();

        var emdrQueue = async.queue(function (task, next) {
            MarketDataRaws
                .updateFromEMDR(task)
                .then(function (updates) {
                    updates.forEach(function (update) {
                        var task = update.pick('resultType', 'typeID', 'regionID');
                        logger.debug("EMDR " + util.inspect(task) + " " +
                            emdrQueue.length());
                        var result_type = update.get('resultType');
                        if ('orders' === result_type) {
                            marginsQueue.push(task);
                        } else if ('history' === result_type) {
                            historyQueue.push(task);
                        }
                    });
                    return next();
                });
        ...
        var historyQueue = async.queue(function (task, next) {
            MarketHistoryAggregates
        ...
        var marginsQueue = async.queue(function (task, next) {
            MarketMargins
                .updateFromMarketData(task.typeID, task.regionID)
                .then(function (margins) {
                    logger.debug("Margins for " + util.inspect(task) + " " + 
                        marginsQueue.length());
                    if (margins.length > 0) {
                        leadsQueue.push(task);
                    }
                    return next();
                });
        }, 1);

        var leadsQueue = async.queue(function (task, next) {
            MarketTradeLeads
                .updateFromMarketData(task.typeID, task.regionID)
                .then(function (results) {
        ...
        sock.on('message', function (msg) {
            zlib.inflate(msg, function (err, market_json) {
                emdrQueue.push(JSON.parse(market_json));
            });
        });
...
prog.command('tradescan [solar system name]')
    .description('Scan market orders in a station for trade leads')
    .action(init(tradescan));

function tradescan (solar_system_name) {
    eveData.mapSolarSystems({
        solarSystemName: solar_system_name
    }).then(function (systems) {
        return eveData.staStations({
            solarSystemID: _.pluck(systems, 'solarSystemID')
        }).then(function (stations) {
            return Promise.all(stations.map(scanStation));
        });
    }).then(function () {
...
function scanStation (station) {
    var finished = Promise.pending();
    logger.info("Scanning station - " + station.stationName);
    models.db_Main('MarketOrders').select()
        .where({
        ...
        .then(function (types) {
            logger.info(types.length + " types sold in " + station.stationName);
            var next = function () {
                return setImmediate(function () {
                    if (types.length === 0) return finished.fulfill();
                    scanStationAndType(station, types.shift(), next);
                });
            };
            next();
        });
...
function scanStationAndType (station, type, next) {
    models.db_Main('MarketOrders').select().where({
        stationID: station.stationID,
        typeID: type.typeID,
        orderState: 0
    }).then(function (orders) {
        logger.debug("Found " + orders.length + " for " + type.typeName +
            " in " + station.stationName);
        models.marketTradeLeadsQueue.push({
            typeID: type.typeID,
            rows: orders,
            scanNonHubs: true,
            squelchQueueStatus: true
        }, next);
    });
}
