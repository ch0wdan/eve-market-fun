...
prog.command('marketorders:import [path]')
    ...
    .action(init(function (path) {
        ...
        p_fs.readdirAsync(path).then(function (files) {
            // Look for the newest market orders export by filename, parse the
            // CSV if found.
            ...
            // Report the parsing progress.
            logger.info("Parsed %s orders, importing...", data.length);
            return data;
        }).map(function (order) {
            // Look up a character model to annotate each order.
            ...
            // Update the database for each market order.
            return models.MarketOrder.forge({
                orderID: order.orderID,
                characterUuid: order.character.get('uuid')
            }).createOrUpdate(_.pick(order, [
                'typeID', 'charID', 'regionID', 'stationID', 'range', 'bid',
                'price', 'volEntered', 'volRemaining', 'issueDate',
                'orderState', 'minVolume', 'accountID', 'duration', 'isCorp',
                'solarSystemID', 'escrow'
            ]));
        }).all().then(function (models) {
            // Finalize all the things and report the result.
