...
function cmd_trade () {
    ...
    var mkfilter = function (station, bid, type) {
        return function (qb) {
            qb.andWhere('stationID', '=', station);
            qb.andWhere('typeID', '=', type.typeID);
            qb.andWhere('volRemaining', '>', 0);
            qb.andWhere('orderState', '=', 0);
            qb.andWhere('bid', '=', bid ? 1 : 0);
            if (bid) {
            ...
            models.MarketOrders.forge()
                ...
                .then(function (orders) {
                    ...
                    if (from_orders.length && to_orders.length) {
                        ...
                        logger.debug(
                            "\t " + 
                            top_from.volRemaining + " @ " + top_from.price +
                            " -> " + 
                            top_to.volRemaining + " @ " + top_to.price);
