        ...
        sock.on('message', function (msg) {
            zlib.inflate(msg, function (err, market_json) {
                ...
                try {
                    ...
                    logger.error('processEmuu failed', e);
                }
            });
        });
...
prog.command('trade')
    .description('Find trade routes')
    .action(init(cmd_trade));

function round(num, places) {
    places = places || 2;
    var pad = Math.pow(10, places);
...
function cmd_trade () {
    ...
    var from_systemid = '30002510';
    var from_bid = false;

    var to_station = '60003760';
    var to_systemid = '30000142';
    var to_bid = true;
    
    var min_margin = 1.80;
    var min_profit = 1000000;
    var max_volume = 40000;
    var max_age = 3;
    ...
    var mkfilter = function (systemid, bid, type) {
        return function (qb) {
            qb.andWhere('typeID', '=', type.typeID)
                .andWhere('solarSystemID', '=', systemid)
                .andWhere('volRemaining', '>', 0)
                .andWhere('orderState', '=', 0)
                .andWhere('bid', '=', bid ? '1' : '0');
            if (bid) {
    ...
    // eveData.invTypes().then(function (types) {

    models.db_Main('MarketOrders')
        .select()
        .where('solarSystemID', '=', from_systemid)
        .andWhere('bid', '=', from_bid)
        .join('invTypes', 'invTypes.typeID', '=', 'MarketOrders.typeID')
        .groupBy('MarketOrders.typeID')
        .then(function (types) {
        ...
        var next = function () {
            ...
            var from_orders, to_orders;

            models.MarketOrders.forge()
            .query(mkfilter(from_systemid, from_bid, type)).fetch()
            .then(function (orders) {
                
                from_orders = orders.toJSON();
                if (!from_orders.length) { throw 'no from orders'; }

                return models.MarketOrders.forge()
                    .query(mkfilter(to_systemid, to_bid, type)).fetch();

            })
            .then(function (orders) {

                to_orders = orders.toJSON();
                if (!to_orders.length) { throw 'no to orders'; }

                if (from_orders[0].price >= to_orders[0].price) { throw 'negative margin'; }

                var top_from = from_orders[0];
                var top_to = to_orders[0];

                var from_age = round((Date.now() - top_from.updated_at) / 3600000);
                var to_age = round((Date.now() - top_to.updated_at) / 3600000);
                if (from_age > max_age || to_age > max_age) { throw 'data too old'; }

                var trade_vol = (top_from.volRemaining > top_to.volRemaining) ?
                    top_to.volRemaining : top_from.volRemaining;

                var volume = Math.round(trade_vol * type.volume * 100) / 100;
                // if (volume > max_volume) { throw 'too much volume'; }

                var from_price = top_from.price;
                var to_price = top_to.price;
                var margin = Math.round( (to_price - from_price) * 100) / 100;
                var perc = Math.round( (margin / to_price) * 10000 ) / 100;

                if (perc < min_margin) { throw 'low margin'; }

                var base_profit = Math.round( (trade_vol * margin) * 100) / 100;
                // if (base_profit < min_profit) { throw 'low profit'; }

                var upfront = Math.round( (from_price * trade_vol) * 100) / 100;
                
                logger.debug(type.typeName);
                logger.debug("\t " + perc + "% / " + margin + ";" +
                    " " + trade_vol + " * " + type.volume + "m3 = " + volume + "m3;" +
                    " " + base_profit + " from " + upfront);
                for (var i=0; i<5; i++) {
                    logger.debug(
                        "\t " + 
                        from_orders[i].volRemaining + " @ " + from_orders[i].price +
                        " " + round((Date.now() - from_orders[i].updated_at) / 1000 / 3600) + "h " +
                        " -> " + 
                        to_orders[i].volRemaining + " @ " + to_orders[i].price +
                        " " + round((Date.now() - to_orders[i].updated_at) / 1000 / 3600) + "h "
                        );
                }

            })
            .catch(function (e) {
                //logger.error(type.typeName + ": " + e);
            })
            .finally(function () {
                next();
            });
