...
prog.command('marketorders:import [path]')
    ...
    .action(init(function (path) {
        ...
        var fresh_import_fns = _.chain(fs.readdirSync(path))
            .map(function (fn) {
        ...
        Promise.all(_.map(fresh_import_fns, function (fn) {
            ...
            var full_fn = conf.get('marketlogs_path') + '/' + fn;
    ...
    .action(init(function () {
        ...
        sock.on('message', function(msg){
            ...
            zlib.inflate(msg, function(err, market_json) {
                ...
                //if ('orders' != market_data) return;

                // Do something useful
                util.debug(util.inspect(market_data,{depth:null}));
            });
        });
    }));
