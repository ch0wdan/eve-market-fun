...
prog.command('marketorders:import [path]')
    ...
    .action(init(function (path) {
        ...
        // Cached character ID -> UUID fetch utility.
        var character_cache = {};
        var fetchCharacterUUID = function (char_id) {
        ...
        var queue = async.queue(function (record, next) {
            ...
            fetchCharacterUUID(char_id)
                .then(function (character_uuid) {
                    record.characterUuid = character_uuid;
                ...
                .then(function (order) {
                    imported_ct++;
                    if ((imported_ct % 1000) == 0) {
                        logger.verbose("%s records saved...", imported_ct);
                    }
                    next();
                })
                .catch(function (e) {
        ...
        // Every time the queue drains, streams get unpaused. If all streams
        // are exhausted, exit.
        queue.drain = function () {
            if (_.keys(streams).length) {
        ...
        var streams = _.chain(fresh_import_fns).map(function (fn) {
            ...
            var stream = CSV().from.path(fn)
                ...
                .on('end', function (count) {
                    delete streams[fn];
                    logger.info("Total of %s records parsed from %s", count, fn);
                })
                .on('record', function (row, index) {
                    ...
                    parsed_ct++;
                    if ((parsed_ct % 1000) == 0) {
                        logger.verbose("%s records parsed...", parsed_ct);
                    }
                });
            return [fn, stream];
        }).object().value();
    ...
    .action(init(function () {
        ...
        sock.on('message', function(msg){
            ...
            zlib.inflate(msg, function(err, market_json) {
                ...
                // Barleguet and Dodixie
                if ([10000048, 10000032].indexOf(market_data.rowsets[0].regionID) == -1) return;
                ...
                Promise.all(_.map(market_data.rowsets, function (rowset) {
                    return eveData.invTypes({typeID: rowset.typeID})
                        ...
                        .spread(function (item, orders) {
                            logger.info("%s received for %s (%s) @ %s",
                                orders.length, item.typeName, item.typeID, rowset.regionID);
                        });
                }));
            });
        });
