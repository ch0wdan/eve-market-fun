...
module.exports = function (app) {
    ...
    app.get('/data/market/type/:typeID', function (req, res) {
        models.MarketOrders.forge().query(function (qb) {
            qb.whereRaw('updated_at > date_sub(now(), interval 6 day)')
                .andWhere('typeID', '=', req.params.typeID)
                .andWhere('orderState', '=', 0);
            if ('bid' in req.query) {
                qb.andWhere('bid', '=', req.query.bid);
            }
            if (req.query.regionID) {
                qb.whereIn('regionID', utils.coerceArray(req.query.regionID));
            }
            if (req.query.solarSystemID) {
                qb.whereIn('solarSystemID', utils.coerceArray(req.query.solarSystemID));
            }
